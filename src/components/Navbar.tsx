    ...
    <>
      <motion.nav
        ...
        className={`fixed z-50 transition-all duration-500 ${isTrackingPage
          ? "hidden md:flex md:top-0 md:left-0 md:bottom-0 md:w-24 md:h-full md:border-r border-white/10 bg-slate-950/95 text-white shadow-2xl backdrop-blur-3xl overflow-y-auto overflow-x-hidden"
          ...
          <div className={`flex ${isTrackingPage ? "flex-col gap-6" : "hidden md:flex items-center gap-2"}`}>
              ...
              return (
                <Link
                  ...
                  className={`transition-all flex items-center gap-2 ${isTrackingPage
                    ? `p-3 rounded-2xl ${isActive ? "bg-primary text-primary-foreground shadow-lg shadow-primary/30" : "text-white/70 hover:bg-white/10 hover:text-white"}`
            ...
            {isTrackingPage && (
              <button
                ...
                title="Trip Status"
                className="p-3 rounded-2xl text-white/70 hover:bg-white/10 hover:text-white transition-all mt-2 group"
              >
            ...
            {user ? (
              <div className={`flex ${isTrackingPage ? "flex-col gap-6" : "hidden md:flex items-center gap-4"}`}>
                ...
                {!isTrackingPage ? (
                  ...
                  <button onClick={handleLogout} className="p-3 text-white/70 hover:text-destructive transition-colors" title="Logout">
              ...
              <div className={`flex ${isTrackingPage ? "flex-col gap-4" : "hidden md:flex items-center gap-2"}`}>
                ...
                {!isTrackingPage && (
                  <Link to="/signup">
                    <Button variant="secondary" size="sm" className="px-6">Sign Up</Button>
                  </Link>
                )}
              </div>
            )}
            ...
            <div className="md:hidden flex items-center gap-3">
              ...
              {user ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Link to="/signup">
                    <Button variant="secondary" size="sm" className="h-9 px-4 rounded-xl text-xs font-bold shadow-lg">Sign Up</Button>
                  </Link>
                </div>
              )}
